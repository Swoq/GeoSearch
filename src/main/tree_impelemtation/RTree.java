...
public class RTree implements SpatialIndex {
    ...
    public void init(Properties props) {
        if (props == null) {
            ...
            if (maxNodeEntries < 2) {
                System.out.println(("Invalid MaxNodeEntries = " + maxNodeEntries + " Resetting to default value of " + DEFAULT_MAX_NODE_ENTRIES));
                maxNodeEntries = DEFAULT_MAX_NODE_ENTRIES;
            }

            // The MinNodeEntries must be less than or equal to (int) (MaxNodeEntries / 2)
            if (minNodeEntries < 1 || minNodeEntries > maxNodeEntries / 2) {
                System.out.println(("MinNodeEntries must be between 1 and MaxNodeEntries / 2"));
        ...
        System.out.println(("R-tree: " + " MaxNodeEntries = " + maxNodeEntries + ", MinNodeEntries = " + minNodeEntries));
    }

    public void addFromData(Data data){
        for(int i = 0; i<data.size(); i++) {
            Data.Location location = data.getLocationByIndex(i);
            Rectangle rectangle = new Rectangle(location.getX(), location.getY());
    ...
    public Rectangle getRectangleById(int k){
        return rectangles.get(k);
    }
    ...
    public void nearestLocations(Point p, TIntProcedure v, int count, float furthestDistance) {
        createNearestNDistanceQueue(p, count, furthestDistance);
    ...
    private Node splitNode(Node n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId) {
        ...
        if (INTERNAL_CONSISTENCY_CHECKING) {
            ...
            if (!nMBR.equals(calculateMBR(n))) {
                System.out.println(("Error: splitNode old node MBR wrong"));
            }
            ...
            if (!newNodeMBR.equals(calculateMBR(newNode))) {
                System.out.println(("Error: splitNode new node MBR wrong"));
            }
        }
    ...
    private void pickSeeds(Node n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId, Node newNode) {
        ...
        for (int i = 0; i < n.entryCount; i++) {
            ...
            if (normalizedSeparation > 1 || normalizedSeparation < -1) {
                System.out.println(("Invalid normalized separation X"));
            }
        ...
        for (int i = 0; i < n.entryCount; i++) {
            ...
            if (normalizedSeparation > 1 || normalizedSeparation < -1) {
                System.out.println(("Invalid normalized separation Y"));
            }
    ...
    private int pickNext(Node n, Node newNode) {
        ...
        for (int i = 0; i < maxNodeEntries; i++) {
            if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
                ...
                if (n.ids[i] == -1) {
                    System.out.println(("Error: Node " + n.nodeId + ", entry " + i + " is null"));
                }
    ...
    private boolean checkConsistency(int nodeId, int expectedLevel, Rectangle expectedMBR) {
        ...
        if (nodeId == rootNodeId && size() == 0) {
            if (n.level != 1) {
                System.out.println(("Error: tree is empty but root node is not at level 1"));
                return false;
            }
        }

        if (n.level != expectedLevel) {
            System.out.println(("Error: Node " + nodeId + ", expected level " + expectedLevel + ", actual level " + n.level));
            return false;
        }
        ...
        if (!actualMBR.equals(calculatedMBR)) {
            System.out.println(("Error: Node " + nodeId + ", calculated MBR does not equal stored MBR"));
            if (actualMBR.minX != n.mbrMinX) System.out.println(("  actualMinX=" + actualMBR.minX + ", calc=" + calculatedMBR.minX));
            if (actualMBR.minY != n.mbrMinY) System.out.println(("  actualMinY=" + actualMBR.minY + ", calc=" + calculatedMBR.minY));
            if (actualMBR.maxX != n.mbrMaxX) System.out.println(("  actualMaxX=" + actualMBR.maxX + ", calc=" + calculatedMBR.maxX));
            if (actualMBR.maxY != n.mbrMaxY) System.out.println(("  actualMaxY=" + actualMBR.maxY + ", calc=" + calculatedMBR.maxY));
            return false;
        }

        if (expectedMBR != null && !actualMBR.equals(expectedMBR)) {
            System.out.println(("Error: Node " + nodeId + ", expected MBR (from parent) does not equal stored MBR"));
            return false;
        }
        ...
        if (expectedMBR != null && actualMBR.sameObject(expectedMBR)) {
            System.out.println(("Error: Node " + nodeId + " MBR using same rectangle object as parent's entry"));
            return false;
        }

        for (int i = 0; i < n.entryCount; i++) {
            if (n.ids[i] == -1) {
                System.out.println(("Error: Node " + nodeId + ", Entry " + i + " is null"));
                return false;
            }
